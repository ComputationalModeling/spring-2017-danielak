...
- Call [one of the `numpy` math functions] on your x-values to generate your y-values. Use a different `numpy` math function for each of your four plots
...
my_x_values = np.linspace(0, 58, 300) # generate 300 points including 0 and 58
my_y_values = np.sin(my_x_values)

plt.plot(my_x_values, my_y_values, '-o') # The '-o' makes it a line plot with dots
...
`numpy` Comes [pre-loaded with math functions](). If you want to compute the sine of $\pi/2$, for example:
...
That makes sense, because sin(0) = 0, and sin(pi / 2) = 1
"""
```
